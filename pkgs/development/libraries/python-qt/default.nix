...
let
  qtVersion = lib.versions.major (lib.getVersion qtbase);
  pyVersion = lib.versions.majorMinor (lib.getVersion python3);
in
stdenv.mkDerivation (finalAttrs: {
  ...
  preConfigure = lib.optionalString qt6Support (
    ...
    ''
      ...
      ./pythonqt_generator --include-paths=${includePaths} qtscript_masterinclude.h build_all.txt
      popd
    ''
  );
  ...
  preFixup = lib.optionalString stdenv.hostPlatform.isDarwin ''
    install_name_tool -id \
      $out/lib/libPythonQt-Qt${qtVersion}-Python${pyVersion}.dylib \
      $out/lib/libPythonQt-Qt${qtVersion}-Python${pyVersion}.dylib
    install_name_tool -change \
               libPythonQt-Qt${qtVersion}-Python${pyVersion}.3.dylib \
      $out/lib/libPythonQt-Qt${qtVersion}-Python${pyVersion}.3.dylib \
      -id \
      $out/lib/libPythonQt_QtAll-Qt${qtVersion}-Python${pyVersion}.dylib \
      $out/lib/libPythonQt_QtAll-Qt${qtVersion}-Python${pyVersion}.dylib
  '';
